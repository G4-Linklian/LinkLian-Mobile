...
import 'package:LinkLian/features/auth/presentation/controllers/auth_controller.dart';
import 'package:LinkLian/features/class_feed/data/repositories/class_feed_repository.dart';

// Mock classes for proper unit testing
class MockAssignmentRepository extends GetxService implements AssignmentRepository {
  List<dynamic> mockAssignments = [];
  List<dynamic> mockStudents = [];
  List<dynamic> mockGroups = [];
  Map<String, dynamic>? mockSubmissionDetail;
  Map<String, dynamic>? mockPostDetail;
  bool shouldThrowError = false;
  
  @override
  Future<List<dynamic>> getClassAssignments({
  ...
  @override
  Future<List<dynamic>> searchAssignments({
    required int sectionId,
    ...
    return mockAssignments
        .where((a) => a['title']?.toLowerCase()?.contains(keyword.toLowerCase()) ?? false)
        .take(limit)
        .toList();
  }
  
  @override
  Future<List<dynamic>> getStudentsSubmissionStatus({
  ...
  @override
  Future<dynamic> getSubmissionDetail({
  ...
  @override
  Future<List<dynamic>> getAllGroups({required int assignmentId}) async {
    if (shouldThrowError) throw Exception('Groups Error');
    return mockGroups;
  }
  
  @override
  Future<dynamic> getPostAssignment({
    required int postId,
    String? role,
  }) async {
...
class MockSubmissionRepository extends GetxService implements SubmissionRepository {
  bool shouldThrowError = false;
  Map<String, dynamic>? mockSubmissionResult;
  ...
  Future<dynamic> createSubmission({
    ...
    return mockSubmissionResult ?? {'id': 1, 'assignmentId': assignmentId};
  }
  ...
  Future<dynamic> updateSubmission({
    ...
    return mockSubmissionResult ?? {'id': submissionId, 'assignmentId': assignmentId};
  }
...
class MockClassFeedRepository extends GetxService implements ClassFeedRepository {
  List<dynamic> mockEducators = [];
  bool shouldThrowError = false;
  
  @override
  Future<List<dynamic>> getSectionEducators({required int sectionId}) async {
...
class MockAuthController extends GetxController {
  ...
  String get role => mockRole;
  int get userId => mockUserId;
}
  ...
  group('Assignment Controllers Unit Tests', () {
    ...
    group('ClassAssignmentController Tests', () {
      ...
      test('should fetch assignments successfully', () async {
        mockAssignmentRepo.mockAssignments = [
          {'id': 1, 'title': 'Assignment 1', 'submissionStatus': 'ยังไม่ส่ง'},
          {'id': 2, 'title': 'Assignment 2', 'submissionStatus': 'ส่งแล้ว'},
        ];
      ...
      test('should apply filters correctly', () async {
        mockAssignmentRepo.mockAssignments = [
          {'id': 1, 'title': 'Assignment 1', 'submissionStatus': 'ยังไม่ส่ง'},
          {'id': 2, 'title': 'Assignment 2', 'submissionStatus': 'ส่งแล้ว'},
          {'id': 3, 'title': 'Assignment 3', 'submissionStatus': 'ส่งช้า'},
        ];
        ...
        expect(controller.filteredAssignments.length, equals(1));
        expect(controller.filteredAssignments.first['submissionStatus'], equals('ส่งแล้ว'));
      });
      ...
      test('should handle pagination correctly', () async {
        // Setup initial data
        mockAssignmentRepo.mockAssignments = List.generate(25, (i) => {
          'id': i + 1,
          'title': 'Assignment ${i + 1}',
          'submissionStatus': 'ยังไม่ส่ง'
        });
    ...
    group('SearchAssignmentController Tests', () {
      ...
      test('should perform search with valid keyword', () async {
        ...
        expect(controller.results.length, equals(2)); // Math Assignment + Math Quiz
      });
      ...
      test('should clear results for empty keyword', () async {
        controller.init(sectionId: 123, role: 'student');
        controller.results.addAll([{'id': 1}, {'id': 2}]);
    ...
    group('TeacherSubmissionController Tests', () {
      ...
      setUp(() {
        Get.arguments = {'assignmentId': 123, 'maxScore': 100.0};
      ...
      test('should calculate submission statistics correctly', () async {
        mockAssignmentRepo.mockStudents = [
          {'hasSubmitted': true, 'submittedAt': DateTime.now().toString()},
          {'hasSubmitted': true, 'submittedAt': DateTime.now().toString()},
      ...
      test('should filter submissions by status', () async {
        mockAssignmentRepo.mockStudents = [
          {'hasSubmitted': true, 'displayName': 'Student A'},
          {'hasSubmitted': false, 'displayName': 'Student B'},
          {'hasSubmitted': true, 'displayName': 'Student C'},
        ];
    ...
    group('AssignmentSubmissionController Tests', () {
      ...
      setUp(() {
        Get.arguments = {'postId': 123};
        mockAuthController.mockRole = 'student';
      ...
      test('should fetch assignment details successfully', () async {
        mockAssignmentRepo.mockPostDetail = {
          'id': 123,
      ...
      test('should validate link attachments correctly', () async {
        ...
        expect(controller.uploadedFiles.length, equals(1)); // Still only one valid link
      });
      ...
      test('should detect teacher role correctly', () {
        mockAuthController.mockRole = 'teacher';
        expect(controller.isTeacher, isTrue);
        
        mockAuthController.mockRole = 'student';
        expect(controller.isTeacher, isFalse);
        
        mockAuthController.mockRole = 'instructor';
        expect(controller.isTeacher, isTrue);
      });
      ...
      test('should enforce file modification permissions', () {
        ...
        controller.submission.value = {'id': 1};
      ...
      test('should detect teacher role correctly', () {
        ...
        bool isTeacher = roleValue.value == 'teacher' || roleValue.value == 'instructor';
        bool isStudent = roleValue.value == 'high school student' || roleValue.value == 'uni student';
        
        expect(isTeacher, isTrue);
        expect(isStudent, isFalse);
        
        roleValue.value = 'instructor';
        isTeacher = roleValue.value == 'teacher' || roleValue.value == 'instructor';
        expect(isTeacher, isTrue);
      });
      ...
      test('should detect student role correctly', () {
        ...
        isStudent = roleValue.value == 'high school student' || roleValue.value == 'uni student';
        expect(isStudent, isTrue);
      });
      ...
      test('should provide correct filter options based on role', () {
        ...
        List<String> getFilterOptions(String role) {
          if (role == 'high school student' || role == 'uni student' || role == 'student') {
      ...
      test('should filter student assignments correctly', () {
        ...
        List<Map<String, dynamic>> filterItems(List<Map<String, dynamic>> items, String filter) {
          switch (filter) {
            case 'ส่งช้า':
              return items.where((a) =>
                a['student_status'] == 'ส่งแล้วเกินกำหนด' ||
                a['student_status'] == 'ยังไม่ส่งเกินกำหนด').toList();
            case 'ยังไม่ส่ง':
              return items.where((a) =>
            ...
            case 'ส่งแล้ว':
              return items.where((a) =>
                ...
                a['student_status'] == 'ส่งแล้วเกินกำหนด').toList();
            default:
              return items;
          }
        }
    ...
    group('SearchAssignmentController Business Logic Tests', () {
      ...
      test('should simulate search filtering', () {
        ...
        List<Map<String, dynamic>> searchAssignments(List<Map<String, dynamic>> assignments, String keyword) {
          return assignments.where((a) =>
            a['title'].toString().toLowerCase().contains(keyword.toLowerCase())
          ).toList();
        }
    ...
    group('TeacherSubmissionController Business Logic Tests', () {
      ...
      test('should filter submissions by status', () {
        ...
        List<Map<String, dynamic>> filterByStatus(List<Map<String, dynamic>> submissions, String filter) {
          switch (filter) {
            case 'submitted':
              return submissions.where((s) => s['has_submitted'] == true).toList();
            case 'not_submitted':
              return submissions.where((s) => s['has_submitted'] == false).toList();
            default:
              return submissions;
          }
        }
    ...
    group('AssignmentSubmissionController Business Logic Tests', () {
      ...
      test('should validate group submission requirements', () {
        bool canSubmitGroup(String groupName, List<int> selectedStudents, bool isSubmitting) {
      ...
      test('should filter students by search query', () {
        ...
        List<Map<String, dynamic>> filterStudents(List<Map<String, dynamic>> students, String query) {
    ...
    group('Utility Functions Tests', () {
      ...
      test('should validate assignment data correctly', () {
        bool validateAssignmentData(Map<String, dynamic> data) {
          if (!data.containsKey('assignment_id') || data['assignment_id'] is! int) {
            return false;
          }
        ...
        expect(validateAssignmentData({'assignment_id': 1, 'max_score': 100.0}), isTrue);
        expect(validateAssignmentData({}), isFalse);
        expect(validateAssignmentData({'assignment_id': 'invalid'}), isFalse);
        expect(validateAssignmentData({'assignment_id': 1, 'max_score': 'invalid'}), isFalse);
      });
      ...
      test('should handle assignment status calculations', () {
        Map<String, int> calculateAssignmentStats(List<Map<String, dynamic>> assignments) {
    ...
    group('Edge Cases and Error Handling Tests', () {
      ...
      test('should handle empty lists gracefully', () {
        List<Map<String, dynamic>> filterEmptyList(List<Map<String, dynamic>> items, String filter) {
